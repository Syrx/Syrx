namespace Syrx.Commanders.Databases.Extensions.Configuration.Tests.Unit.TypeSettingOptionsBuilderTests
{
    public class ForType
    {
        private TypeSettingOptionsBuilder<ForType> _builder;
        private const string CommandText = "test-command-text";
        private const string Alias = "test-alias";

        public ForType()
        {
            _builder = new TypeSettingOptionsBuilder<ForType>();
        }
        ...
        public void ForSpecifiedType()
        {
            ...
            Action<CommandSettingOptionsBuilder> builder = (x) => x.UseCommandText(CommandText).UseConnectionAlias(Alias);
        ...
        {
            Action<CommandSettingOptionsBuilder> builder = (x) => x.UseConnectionAlias(Alias).UseCommandText(CommandText);
